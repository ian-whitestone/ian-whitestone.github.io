...
We'll have a single Compute Engine instance (with 16 vCPUs) running in GCP. The instance will be running a single docker container which contains our `dask-scheduler` process & 16 `dask-worker` processes. Our local Python session/development environment can then send tasks to this single node cluster for much faster execution.
