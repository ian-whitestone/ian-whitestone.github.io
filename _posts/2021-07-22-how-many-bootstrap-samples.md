...
For a recent causal inference problem, I used a metric called "average treatment effect on the treated" (ATT)<sup>1</sup> to measure the impact of a new product launch. ATT gives you the expected impact of the treatment, the effect you are modelling, on the outcome (i.e. conversion rate), for the population who received the treatment. It's calculated by taking the treated population, and predicting what would have happened if they did not receive the treatment, and comparing that to the predictions of what would happen if they did.
...
And then instead of running this function sequentially 10,000 times (slow!), we can leverage [Dask](https://dask.org/) to distribute this work across all available CPU cores (my Macbook Pro has 16).

```python
from dask.distributed import Client, LocalCluster
# I only used 12 workers (instead of 16) since you'll need enough
...
And voila! We know have 10,000 bootstrapped ATTs in `bootstrap_results.txt`.
...
### When do our bootstrapped metrics stop changing?

Now that we have these samples, we can see how our uncertainty estimates of ATT change after each additional sample. For example, calculate our 95th percentile using 10 bootstrap samples, then 11, 12, all the way up to 10,000. We can also calculate the % difference at each step to understand how much each metric is changing with each new sample added.
...
# See how much each metric changes with the newly added sample
...
We can now easily plot these to see how quickly they stabilize. Here's what our metrics look like as we go from 1 to 100 bootstrap samples. In the first 50 they are changing quite a bit and then begin to stabilize. 
...
3. When researching pitfalls of bootstrapping, you'll come across [articles](https://yanirseroussi.com/2019/01/08/hackers-beware-bootstrap-sampling-may-be-harmful/) that say you need to be doing this thousands of times for certain statistics. 
    * As an aside, if someone has a better article explaining the pitfalls of boostrapping please send it my way!